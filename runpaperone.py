...
import os
with open('top_dig_prompts.txt', 'r') as f:
    prompts = f.readlines()
...
ct = len(prompts)
#make prompt dirs
# for i, prompt in enumerate(prompts[70:]):
#     os.makedirs(f"images/{prompt}", exist_ok=True)
...
data = [['image', 'niqe', 'brisque', 'piqe', 'metaiqa', 'rankiqa', 'hyperiqa', 'contrique', 'cnniqa', 'tres']]
base_path = r'c:/Users/karat/Desktop/responsibleai/niqatest'
for prompt in prompts:
    for i in range(count):
        for txt in ['dalle_image_', 'imagen_image_', 'kandinsky_image_', 'sd_image_']:
            imgpath = f"{base_path}/../images/{prompt}/{txt}{i}.png"
            results = get_metrics(imgpath, base_path)

            data.append([f"{prompt}_{txt}{i}"] + results)
